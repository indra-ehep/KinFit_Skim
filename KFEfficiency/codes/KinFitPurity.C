...
int KinFitPurity()
{
  ...
  string sample_extn = "CBA_gdjsoncorr";
  ...
  string input_hist_file = "/Data/CMS-Analysis/NanoAOD-Analysis/SkimAna/root_files/grid_v40_Syst/"+sample_extn+"/post/all_TTbar.root";
  ...
  TH1D *hWtBefore = (TH1D *)finhist->Get(Form("%s/%s/Iso/_wt_before_mu",sig.c_str(),syst.c_str()));
  TH1D *hWtAfter =  (TH1D *)finhist->Get(Form("%s/%s/Iso/_wt_after_mu",sig.c_str(),syst.c_str()));
  ...
  tr->SetBranchAddress("lhe1dR", &lhe1dR);
  tr->SetBranchAddress("lhe2dR", &lhe2dR);
  ...
  //for (Long64_t ievent = 0 ; ievent < 5 ; ievent++ ) {
    ...
    //if(singleMu and !singleEle and muonIsoCut and met.Pt()>20.0){
      ...
      // if(chi2<1.) { //chi^2<2 ~= DelPhi>2.7 // chi^2<10 ~= DelPhi>1.5 // chi^2<5 ~= DelPhi>2.2
	 ...
	 if(chi2<5. and TMath::Abs(top.DeltaPhi(tbar))>2.5) { //chi^2<2 ~= DelPhi>2.7 // chi^2<10 ~= DelPhi>1.5 // chi^2<5 ~= DelPhi>2.2
	   mjj_cut4->Fill(mjjKF,weight*wt_ratio);
	   hDelPhi->Fill(TMath::Abs(top.DeltaPhi(tbar)));
	 }//else	

	if(lhe1dR < 0.2 and lhe2dR < 0.2)
  ...
  TFile *fout = new TFile(Form("output/output_purity.root"),"recreate");
  mjj->Write();
  mjj_cut1->Write();
  mjj_cut2->Write();
  mjj_cut3->Write();
