...
int BCRegression(string infile = "/Data/root_files/AOD_files/NanoAODUL/2016/TTbarPowheg_Semilept_postVFP_Skim_NanoAOD_1of27.root")
{
  ...
  Float_t         jetbRegCorr_[200];
  Float_t         jetcRegCorr_[200];
  ...
  tree->SetBranchAddress("Jet_bRegCorr", &jetbRegCorr_);

  tree->SetBranchStatus("Jet_cRegCorr",1);
  tree->SetBranchAddress("Jet_cRegCorr", &jetcRegCorr_);
  ...
  /////////////////////////////////////////////////
  TLorentzVector pTemp, pTemp1, pTemp2;
  ...
  TH1F *hDiffcGJRJUncorr = new TH1F("hDiffcGJRJUncorr","hDiffcGJRJUncorr", 1000 , -50., 50.);
  TH1F *hDiffcGJRJCorr = new TH1F("hDiffcGJRJCorr","hDiffcGJRJCorr", 1000 , -50., 50.);
  TH1F *hDiffbGJRJUncorr = new TH1F("hDiffbGJRJUncorr","hDiffbGJRJUncorr", 1000 , -50., 50.);
  TH1F *hDiffbGJRJCorr = new TH1F("hDiffbGJRJCorr","hDiffbGJRJCorr", 1000 , -50., 50.);
  ...
  for(int ievent = 0; ievent < tree->GetEntries() ; ievent++){
    ...
    for (unsigned int irec = 0 ; irec < nGenJet_ ; irec++ ){      
      ...
      float rawPt = jetPt_[irec] * (1. - jetRawFactor_[irec]);
      float energyReg = 1.0;
      if(TMath::Abs(jetPartFlvr_[irec])==4)
	energyReg = jetcRegCorr_[irec];
      if(TMath::Abs(jetPartFlvr_[irec])==5)
	energyReg = jetbRegCorr_[irec];
      //float corrpt = rawPt*energyReg;
      float corrpt = jetPt_[irec]*energyReg;
      if(TMath::Abs(jetPartFlvr_[irec])==4){
	//pTemp2.SetPtEtaPhiM(corrpt, jetEta_[irec] , jetPhi_[irec], jetMass_[irec]);
	hDiffcGJRJUncorr->Fill(jetPt_[irec]-GenJet_pt_[igj]);
	hDiffcGJRJCorr->Fill(corrpt-GenJet_pt_[igj]);
      }
      if(TMath::Abs(jetPartFlvr_[irec])==5){
	hDiffbGJRJUncorr->Fill(jetPt_[irec]-GenJet_pt_[igj]);
	hDiffbGJRJCorr->Fill(corrpt-GenJet_pt_[igj]);
	//pTemp2.SetPtEtaPhiM(corrpt, jetEta_[irec] , jetPhi_[irec], jetMass_[irec]);
      }
  ...
  TFile *fout = TFile::Open("output/bcregoutput.root","recreate");
  // dRMuLHEGP->Write();
  // dREleLHEGP->Write();
  // dRTauLHEGP->Write();
  // dRQdLHEGP->Write();
  ...
  // dRQsGJRJ->Write();
  // dRQcGJRJ->Write();
  hDiffcGJRJUncorr->Write();
  hDiffcGJRJCorr->Write();
  hDiffbGJRJUncorr->Write();
  hDiffbGJRJCorr->Write();
