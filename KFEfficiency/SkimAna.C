...
bool SkimAna::FillMCInfo()
{
  ...
  Info("FillMCInfo", "Event : %llu, nLHE : %u", fProcessed, event->nLHEPart_);
  for (unsigned int imc = event->nLHEPart_- 4 ; imc < event->nLHEPart_ ; imc++ ){      
    ...
    Info("FillMCInfo", "\tLHE : [PDG : %4d(%10s), (%5.3f, %5.3f, %5.3f, %5.3f)]", event->LHEPart_pdgId_[imc],
    	 TDatabasePDG::Instance()->GetParticle(event->LHEPart_pdgId_[imc])->GetName(),
    	 event->LHEPart_pt_[imc], event->LHEPart_eta_[imc] , event->LHEPart_phi_[imc], event->LHEPart_mass_[imc]);
  }// mc particle loop
  ...
  for (unsigned int imc = 0 ; imc < event->nGenJet_ ; imc++ ){      
    ...
    if(abs(event->GenJet_partonFlavour_[imc])==abs(lhePDG[0])){
      delRtoID0.insert( pair<double,int> (pTemp.DeltaR(pLHE[0]), imc) );
      //cout <<"\tFound match 0 for " << imc << endl;
    }
    if(abs(event->GenJet_partonFlavour_[imc])==abs(lhePDG[1])){
      delRtoID1.insert( pair<double,int> (pTemp.DeltaR(pLHE[1]), imc) );
      //cout <<"\tFound match 1 for " << imc << endl;
    }
  }
