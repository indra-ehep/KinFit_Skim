...
eosDir = '/cms/store/user/idas/Output/cms-hcs-run2/Skim_NanoAOD'
...
for year in [2016,2017,2018]:
    ...
    for sampleName, nJob in sampleList.items():
        ...
        extraArgs = "%s_Skim_NanoAOD*"%sampleName
        ...
        fileList = subprocess.Popen('xrdfs root://se01.indiacms.res.in ls -R %s/%i | grep -E "%s"'%(eosDir, year, extraArgs),shell=True,stdout=subprocess.PIPE).communicate()[0].split('\n')
        fileList.remove("")
        nFiles = len(fileList)
