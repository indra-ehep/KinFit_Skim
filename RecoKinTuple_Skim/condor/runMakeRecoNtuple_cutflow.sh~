...
echo "./makeRecoNtuple cutflow ${year} ${sample} ${jobNum} . ${!varname}"
./makeRecoNtuple cutflow ${year} ${sample} ${jobNum} . ${!varname}

echo "delete csv" ; rm *.csv
...
condorOutDir=/eos/user/i/idas/Output/cms-hcs-run2/RecoNtuple_Skim_cutflow
if [ -z ${_CONDOR_SCRATCH_DIR} ] ; then
    echo "Running Interactively" ;
else
    xrdcp -f ${sample}*.root root://eosuser.cern.ch/${condorOutDir}/${year}
