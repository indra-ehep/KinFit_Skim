...
int CreatePUJetIDEff()
{
  ...
  int nofSamples = 1;
  const char *Sample[] = {"TTTT"};  
  
  string inputpath = Form("/afs/cern.ch/user/i/idas/CMS-Analysis/NanoAOD-Analysis/PUJetIDEff");
  string outputpath = Form("/afs/cern.ch/user/i/idas/CMS-Analysis/NanoAOD-Analysis/PUJetIDEff");
  ...
  for(int ifile = 0 ; ifile < nofSamples ; ifile++ ){
    ...
    TFile *f = TFile::Open(Form("%s/%s_pujetid_eff_raw_%d.root",inputpath.c_str(),Sample[ifile],year));    
    TFile *fout = new TFile(Form("%s/%s_pujetid_eff_%d.root",outputpath.c_str(),Sample[ifile],year),"recreate");    
