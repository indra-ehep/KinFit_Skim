...
int PrepMjjSystTableV9UL(int year = 2016)
{
  ...
  outFile<<"\\begin{tabular}{  c c c c c  c c c  c  ccc  cc   c  c c c c c   c c c  c c }"<<endl;
  outFile<<"\\hline "<<endl;  
  ...
  outFile<<" Category & Process &  {\\rotatebox{90}{Pileup (corr.)}} & {\\rotatebox{90}{Prefire}} & {\\rotatebox{90}{Lepton}} "
    //<<"& {\\rotatebox{90}{jet \\& $p_\\text{T}^\\text{miss}$ (corr.)}} "
	 ...
	 <<"& {\\rotatebox{90}{top-$p_\\text{T}$ reweight (shape)}} "
...
string GetDifferential(string rowtitle, int ifile, bool inc, int year, bool isKFL, bool isInc, char cType, bool ismu)
{
  ...
  const int  nofSyst = 23;//10;
  
  double muOut[3];
  double eleOut[3];
  double muErr[24]; //nofSyst + stat
  double eleErr[24]; //nofSyst + stat
  double muErrH[24]; //nofSyst + stat
  double eleErrH[24]; //nofSyst + stat
  
  for(int i=0;i<24;i++) muErr[i] = eleErr[i] = eleErrH[i] = muErrH[i] = 0.0 ;
  ...
  double norm_m = muErr[21];
  double norm_e = eleErr[21];
...
int CalcSystTable(int isample, bool isMu, double output[], double error_percnt[], double error_percntH[], int year, bool isKFL = false, bool isIncl = true, char cType = 'L')
{
  ...
  const int  nofSyst = 22;
  const int  nofSystL = 19;
