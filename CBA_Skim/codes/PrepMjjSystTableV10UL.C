...
int PrepMjjSystTableV10UL(int year = 2016)
{
  ...
  outFile<<"\\begin{tabular}{  c c c c c  ccc ccc cc  c  ccc  cc   c  c c c c c   c c c  c c }"<<endl;
  outFile<<"\\hline "<<endl;  
  ...
  outFile<<" Category & Process &  {\\rotatebox{90}{Pileup (corr.)}} & {\\rotatebox{90}{Prefire}} & {\\rotatebox{90}{Lepton}} "
    	 <<"& {\\rotatebox{90}{JES: subtotal pileup (shape)}} & {\\rotatebox{90}{JES: subtotal relative  (shape)}} & {\\rotatebox{90}{JES: subtotal $p_\\text{T}$}} "
    	 <<"& {\\rotatebox{90}{JES: subtotal scale}} & {\\rotatebox{90}{JES: flavour QCD (shape)}} & {\\rotatebox{90}{JES: time-$p_\\text{T}$-$\\eta$ }} "
	 <<"& {\\rotatebox{90}{JER (corr., shape)}} & {\\rotatebox{90}{pileup jet identification}} "    
	 <<"& {\\rotatebox{90}{b\\&c tagging stat. }} "
	 ...
	 <<"& {\\rotatebox{90}{b\\&c tagging Interp.}} & {\\rotatebox{90}{b\\&c tagging Extrap.}} "
    	 <<"& {\\rotatebox{90}{PDF}} "
	 <<"& {\\rotatebox{90}{Renorm. (corr.)}}  & {\\rotatebox{90}{Factor. (corr.)}} "
    	 <<"& {\\rotatebox{90}{ISR (corr.)}}  & {\\rotatebox{90}{FSR (corr., shape)}} "
	 <<"& {\\rotatebox{90}{top-$p_\\text{T}$ reweight }} "
...
string GetDifferential(string rowtitle, int ifile, bool inc, int year, bool isKFL, bool isInc, char cType, bool ismu)
{
  ...
  const int  nofSyst = 28;//10;
  
  double muOut[3];
  double eleOut[3];
  double muErr[29]; //nofSyst + stat
  double eleErr[29]; //nofSyst + stat
  double muErrH[29]; //nofSyst + stat
  double eleErrH[29]; //nofSyst + stat
  
  for(int i=0;i<29;i++) muErr[i] = eleErr[i] = eleErrH[i] = muErrH[i] = 0.0 ;
  ...
  // Pileup(0), Lepton(1), Prefire(2), JES(3), JER(4), PDF(5), Renormalization(6), Factorization(7), ISR(8), FSR(9), PU JetID(10), b\\&c tagging Interpolation(11), b\\&c tagging Extrapolation(12),
  // b\\&c tagging XS DY-b(13), b\\&c tagging XS DY-c(14), b\\&c tagging XS Wjet-c(15), b\\&c tagging statistics(16), top-pt(17), Normalization(18/21), cp5(18), mtop(19), hdamp(20), Statistical(22)
  ...
  // 			   "bcstatup", "bcstatdown", "topptup",  "topptdown"            //38,40
  // 			   "cp5up", "cp5down","mtopup", "mtopdown", "hdampup", "hdampdown", //42,44,46
  //			   "stotpuup", "stotpudown", "stotrelup", "stotreldown",             //48,50
  //			   "stotptup", "stotptdown", "stotscaleup", "stotscaledown",         //52,54
  //			   "flavorqcdup", "flavorqcddown", "timeptetaup", "timeptetadown"   //56,58   
  // };
  // int  sysidlistMu[nofSyst] = {2, 4, 8, 10, 12, 14, 18, 20, 22, 24, 26, 28, 30, 32, 34, 36, 38, 40, 42, 44, 46, 46};
  //int  sysidlistMu[nofSyst] = {2, 4, 8, 48, 50, 52, 54, 56, 58, 12, 14, 18, 20, 22, 24, 26, 28, 30, 32, 34, 36, 38, 40, 42, 44, 46, 46};

  // Pileup(0), Lepton(1), Prefire(2), stotpu(3), stotrel(4), stotpt(5), stotscale(6), flavorqcd(7), timepteta(8), JER(9), PDF(10), Renormalization(11), Factorization(12), ISR(13), FSR(14), PU JetID(15), b\\&c tagging Interpolation(16), b\\&c tagging Extrapolation(17),
  // b\\&c tagging XS DY-b(18), b\\&c tagging XS DY-c(19), b\\&c tagging XS Wjet-c(20), b\\&c tagging statistics(21), top-pt(22), Normalization(23/26), cp5(23), mtop(24), hdamp(25), Statistical(27)
  ...
  double bcjets_stat_m = muErr[21];
  double bcjets_stat_e = eleErr[21];

  double norm_m = muErr[26];
  double norm_e = eleErr[26];
  ...
  if(sampleName.find("QCDdd")==string::npos){
	...
	+ " & " + Form("$%2.1f$",(ismu)?muErr[20]:eleErr[20])
	+ " & " + Form("$%2.1f$",(ismu)?muErr[16]:eleErr[16])
	...
	+ " & " + Form("$%2.1f$",(ismu)?muErr[22]:eleErr[22])
	+ " & " + Form("$%2.1f$",(ismu)?muErr[23]:eleErr[23])
	+ " & " + Form("$%2.1f$",(ismu)?muErr[24]:eleErr[24])
	+ " & " + Form("$%2.1f$",(ismu)?muErr[25]:eleErr[25])
	+ " & " + Form("$%2.1f$",(ismu)?norm_m:norm_e)
	+ " & " + Form("$%2.1f$",(ismu)?muErr[27]:eleErr[27])
	+"\\\\";
    }else if(sampleName.find("HplmiM")!=string::npos){
      combined = string(" & ") + rowtitle
	+ " & " + Form("$%2.1f$",(ismu)?pileup_m:pileup_e) 
	+ " & " + prefire
	+ " & " + Form("$%2.1f$",(ismu)?lepton_m:lepton_e)
...
string GetInclusive(string rowtitle, int ifile, bool inc, int year, bool isKFL, bool isInc, char cType, double *muTot, double *eleTot)
{
  ...
  const int  nofSyst = 25;
  double muOut[3];
  double eleOut[3];
  double error_percnt[28]; //nofSyst + stat
  double error_percntH[28]; //nofSyst + stat

  cout<<"Inclusive sample " << sampleType[ifile-1] <<", RowTitle : " << rowtitle << endl;
  string sampleName = sampleType[ifile-1];

  for(int i=0;i<28;i++) error_percnt[i] = error_percntH[i] = 0.0 ;
  for(int i=0;i<3;i++) muOut[i] = eleOut[i] = 0.0 ;
...
int CalcSystTable(int isample, bool isMu, double output[], double error_percnt[], double error_percntH[], int year, bool isKFL = false, bool isIncl = true, char cType = 'L')
{
  ...
  // int  sysidlistMu[nofSyst] = {2, 4, 8, 10, 12, 14, 18, 20, 22, 24, 26, 28, 30, 32, 34, 36, 38, 40, 42, 44, 46, 46};
  // int  sysidlistEle[nofSyst] = {2, 6, 8, 10, 12, 14, 18, 20, 22, 24, 26, 28, 30, 32, 34, 36, 38, 40, 42, 44, 46, 46};
  // int  sysidlistMuL[nofSystL] = {2, 4, 8, 10, 12, 14, 18, 20, 22, 24, 26, 28, 30, 32, 34, 36, 38, 40, 40};
  // int  sysidlistEleL[nofSystL] = {2, 6, 8, 10, 12, 14, 18, 20, 22, 24, 26, 28, 30, 32, 34, 36, 38, 40, 40};
  
  const int  nofSyst = 27;//22+5(=6-1)
  const int  nofSystL = 24;//19+5(=6-1)
  int  sysidlistMu[nofSyst] = {2, 4, 8, 48, 50, 52, 54, 56, 58, 12, 14, 18, 20, 22, 24, 26, 28, 30, 32, 34, 36, 38, 40, 42, 44, 46, 46};
  int  sysidlistEle[nofSyst] = {2, 6, 8, 48, 50, 52, 54, 56, 58, 12, 14, 18, 20, 22, 24, 26, 28, 30, 32, 34, 36, 38, 40, 42, 44, 46, 46};
  int  sysidlistMuL[nofSystL] = {2, 4, 8, 48, 50, 52, 54, 56, 58, 12, 14, 18, 20, 22, 24, 26, 28, 30, 32, 34, 36, 38, 40, 40};
  int  sysidlistEleL[nofSystL] = {2, 6, 8, 48, 50, 52, 54, 56, 58, 12, 14, 18, 20, 22, 24, 26, 28, 30, 32, 34, 36, 38, 40, 40};

  double error[nofSyst];
  double error_percent[nofSyst];
  
  const int nSystGroups = 27 ; 
  const char*  syst_group_name[] = {"Pileup", "Lepton", "Prefire", "stotpu", //"JES"
				    "stotrel", "stotpt", "stotscale", "flavorqcd",
				    "timepteta", "JER",
				    "PDF", "Renorm", "Facto", "ISR", "FSR",
				    "PU JetID", "bc interp", "bc extrap", "bc dyb", "bc dyc",
				    "bc wjetc", "bc stat", "top pt",
				    "cp5", "mtop", "hdamp", "Norm"};
  ...
  if(baseFileName.find("all_QCDdd")!=string::npos){
    ...
    TH1D *fQCDSyst	=  (TH1D *)finBase->Get(Form("QCDdd/syst/Iso/%s",histName.Data()));
    Norm_iso20 = fQCDSyst->GetBinError(1)/100.;
  }
