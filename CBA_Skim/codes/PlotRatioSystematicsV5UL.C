...
int PlotRatioSystematicsV5UL(int year = 2016, bool isBtag = 0, bool isMu = 0, int htype = 2){
  ...
  files_avmc.push_back(fin_nano_ttg) ; files_avdd.push_back(fin_nano_ttg);
  files_avmc.push_back(fin_nano_tth) ; files_avdd.push_back(fin_nano_tth);
  files_avmc.push_back(fin_nano_ttw) ; files_avdd.push_back(fin_nano_ttw);
  files_avmc.push_back(fin_nano_ttz) ; files_avdd.push_back(fin_nano_ttz);
  ...
  TH1D	*hcf_nano_ttg	    = (TH1D *)fin_nano_ttg->Get(Form("TTG/base/Iso/%s",histname.c_str()));
  TH1D	*hcf_nano_tth	    = (TH1D *)fin_nano_tth->Get(Form("TTH/base/Iso/%s",histname.c_str()));
  TH1D	*hcf_nano_ttw	    = (TH1D *)fin_nano_ttw->Get(Form("TTW/base/Iso/%s",histname.c_str()));
  TH1D	*hcf_nano_ttz	    = (TH1D *)fin_nano_ttz->Get(Form("TTZ/base/Iso/%s",histname.c_str()));
  ...
  TH1D	*hcf_nano_qcd_sys   = (TH1D *)fin_nano_qcd->Get(Form("QCDdd/syst/Iso/%s",histname.c_str()));
  ...
  if(histname.find("_mjj_")!=string::npos){
    ...
    hcf_nano_ttg->Rebin(rebin);
    hcf_nano_tth->Rebin(rebin);
    hcf_nano_ttw->Rebin(rebin);
    hcf_nano_ttz->Rebin(rebin);
  ...
  const char *syst[] = {    "pdf", "isr", /*"fsr",*/ "pu",
			    "prefire", "mueff", "eleeff", "pujetid",
			    "jer", "absmpfb", "absscl", "absstat",
  			    "flavorqcd", "frag", "timepteta", "pudatamc",
			    "puptbb", "puptec1", "puptec2", "pupthf",
			    "puptref", "relfsr", "relbal", "relsample",
  			    "reljerec1", "reljerec2", "reljerhf", "relptbb",
			    "relptec1", "relptec2", "relpthf", "relstatec",
			    "relstatfsr", "relstathf", "singpiecal", "singpihcal",
  			    "bcstat", "bcintp", "bcextp", "bclhemuf",
			    "bclhemur", "bcxdyb", "bcxdyc", "bcxwjc",
  			    "toppt"
  };
  ...
  hcf_nano_bkg->Add(hcf_nano_ttw);
  hcf_nano_bkg->Add(hcf_nano_ttz);
  ...
  hcf_nano_ttg->SetName(Form("%s_TTG",hcf_nano_ttg->GetName()));
  hcf_nano_tth->SetName(Form("%s_TTH",hcf_nano_tth->GetName()));
  hcf_nano_ttw->SetName(Form("%s_TTW",hcf_nano_ttw->GetName()));
  hcf_nano_ttz->SetName(Form("%s_TTZ",hcf_nano_ttz->GetName()));
  ...
  vSystNominal.push_back(hcf_nano_tth);
  vSystNominal.push_back(hcf_nano_ttw);
  vSystNominal.push_back(hcf_nano_ttz);
  ...
  hcf_nano_ttw->SetFillColor(kOrange+1);
  hcf_nano_tth->SetFillColor(kGray);
  hcf_nano_ttz->SetFillColor(kMagenta);
  hcf_nano_dyjets->SetFillColor(kSpring+2);
  hcf_nano_ttg->SetFillColor(kYellow+1);
  hcf_nano_wjets->SetFillColor(kAzure);
  hcf_nano_qcd->SetFillColor(kMagenta-9);
  hcf_nano_qcd_mc->SetFillColor(kMagenta-9);
  hcf_nano_stop->SetFillColor(kYellow-8);
  hcf_nano_ttbar->SetFillColor(kCyan+1);

  //Handle low met
  for(int i=0; i<hcf_nano_ttbar->GetNbinsX(); i++){
    if(!(hcf_nano_ttbar->GetBinContent(i+1)>0.0)) continue;
    //cout<<" "<<hCentral->GetName()<<" ("<<i+1<<"): "<<hCentral->GetBinContent(i+1)<<endl;
    
    double lowedge = hcf_nano_ttbar->GetXaxis()->GetBinLowEdge(i+1);
    double upedge = hcf_nano_ttbar->GetXaxis()->GetBinUpEdge(i);
    
    if(
       (histname.find("met")!=string::npos and histname.find("pt")!=string::npos and TMath::Abs(lowedge) < 20.)
       or
       (histname.find("eta")!=string::npos and (TMath::Abs(lowedge) > 2.4 or TMath::Abs(upedge) > 2.4))
       ){
      hcf_nano_data->SetBinContent(i+1, 0.0);
      hcf_nano_bkg->SetBinContent(i+1, 0.0);
      hcf_nano_sig->SetBinContent(i+1, 0.0);
      hcf_nano_ttbar->SetBinContent(i+1, 0.0);
      hcf_nano_ttg->SetBinContent(i+1, 0.0);
      hcf_nano_tth->SetBinContent(i+1, 0.0);
      hcf_nano_ttw->SetBinContent(i+1, 0.0);
      hcf_nano_ttz->SetBinContent(i+1, 0.0);
      hcf_nano_stop->SetBinContent(i+1, 0.0);
      hcf_nano_wjets->SetBinContent(i+1, 0.0);
      hcf_nano_dyjets->SetBinContent(i+1, 0.0);
      hcf_nano_vbf->SetBinContent(i+1, 0.0);
      hcf_nano_qcd_mc->SetBinContent(i+1, 0.0);
      hcf_nano_qcd->SetBinContent(i+1, 0.0);
    }
  ...
  float legend_pos1[4] = {0.38, 0.69, 0.54, 0.86};
  float legend_pos2[4] = {0.50, 0.69, 0.71, 0.86};
  ...
  leg2->AddEntry(hcf_nano_ttg, Form("#it{t}#bar{#it{t}}#gamma") ,"f");
  leg2->AddEntry(hcf_nano_dyjets, Form("#it{Z}/#gamma + jets") ,"f");
  leg2->AddEntry(hcf_nano_ttz, Form("#it{t}#bar{#it{t}}Z") ,"f");
  leg2->AddEntry(hcf_nano_tth, Form("#it{t}#bar{#it{t}}h") ,"f");  

  TLegend *leg3 = p->BuildLegend();
  for(int i = 0 ; i < 15 ; i++)
    leg3->DeleteEntry() ;// This is to delete default title of hData  
  leg3->AddEntry(hcf_nano_ttw, Form("#it{t}#bar{#it{t}}W") ,"f");  
...
double errBandUp(int iBin, int year, TH1D *hCentral,  TH1D *hQCD, vector<TH1D *> vSystNom, vector<TH1D *> vSystUp){
  ...
  for(unsigned int inom = 0 ; inom < vSystNom.size() ; inom++){
    ...
    if(hNom->GetBinContent(iBin+1)>0.0 and hname.Contains("TTG"))
      errUp += pow(hNom->GetBinContent(iBin+1)*0.061/2,2);
    if(hNom->GetBinContent(iBin+1)>0.0 and hname.Contains("TTH"))
      errUp += pow(hNom->GetBinContent(iBin+1)*0.061/2,2);
    if(hNom->GetBinContent(iBin+1)>0.0 and hname.Contains("TTW"))
      errUp += pow(hNom->GetBinContent(iBin+1)*0.061/2,2);
    if(hNom->GetBinContent(iBin+1)>0.0 and hname.Contains("TTZ"))
      errUp += pow(hNom->GetBinContent(iBin+1)*0.061/2,2);
...
TGraphAsymmErrors *SystGraph(int year, string histname, TH1D *hCentral,  TH1D *hQCD, vector<TH1D *> vSystNom,  vector<TH1D *> vSystUp, vector<TH1D *> vSystDown, 
  ...
  for(int i=0; i<n1; i++){
    ...
    if(histname.find("ele")!=string::npos and histname.find("jet")==string::npos and histname.find("met")==string::npos and histname.find("pt")!=string::npos and TMath::Abs(lowedge) < 26.) continue;
    //if(histname.find("mu")!=string::npos and histname.find("jet")==string::npos and histname.find("met")==string::npos and histname.find("pt")!=string::npos and TMath::Abs(lowedge) < 30.) continue;
...
TH1D *GetUpDownHistDD(vector<TFile *> filelist, const char *syst, const char *updown, string histname)
{
  ...
  const char *sample[] = {"TTbar", "TTG", "TTH", "TTW", "TTZ", "singleTop", "Wjets", "DYjets", "VBFusion", "QCDdd"};
...
TH1D *GetUpDownHistMC(vector<TFile *> filelist, const char *syst, const char *updown, string histname, bool isMu)
{
  ...
  TH1D *h1 = 0x0;
  TH1D *hcf_nano[filelist.size()] ; //_nano_ttbar->Get(Form("TTbar/base/Iso/%s",histname.c_str()));
  const char *sample1[] = {"TTbar", "TTG", "TTH", "TTW", "TTZ", "singleTop", "Wjets", "DYjets", "VBFusion", "MCQCDMu"};
  const char *sample2[] = {"TTbar", "TTG", "TTH", "TTW", "TTZ", "singleTop", "Wjets", "DYjets", "VBFusion", "MCQCDEle"};
