...
int QCDDDABCD(int year = 2016, bool isBtag = 0, bool isMu = 1, int htype = 10)
{
  ...
  for(int i =0; i < 4; i ++) {
    for(int k =1; k <=nBins ; k ++) {
      ...
      h_signal[i]->SetBinContent(k, 0.01) ;
      h_signal[i]->SetBinError(k, 1) ;
      if( h_data[i]->GetBinContent(k)<0) h_data[i]->SetBinContent(k, 0) ;
      if( h_signal[i]->GetBinContent(k)<0) h_signal[i]->SetBinContent(k, 0.01) ;
    }//k-loop on bins
    
  }
  TRooABCD abcd("abcd",Form("abcd %d",year));
