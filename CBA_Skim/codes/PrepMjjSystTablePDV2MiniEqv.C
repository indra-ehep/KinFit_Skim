...
int PrepMjjSystTablePDV2MiniEqv(int year = 2016)
{
  ...
  bool forPaper = false;
  double muTotL[3],eleTotL[3];
  double muTotM[3],eleTotM[3];
  double muTotT[3],eleTotT[3];
  ...
  outFile<<"\\begin{tabular}{  ccc cc c  cc}"<<endl;
  outFile<<"\\hline "<<endl;
  outFile<<" Category & Process &  Pileup "
	 <<"& jet \\& $p_\\text{T}^\\text{miss}$ & b\\&c jets "
	 <<"& Normalization  & Statistical "
	 <<"& $p_{\\text{T}}$ (top)  "
	 <<"\\\\ "
   	 <<""<<endl;
  // outFile<<" Category & Process &  {\\rotatebox{90}{Pileup}} & {\\rotatebox{90}{Lepton}} "
  // 	 <<"& {\\rotatebox{90}{jet \\& $p_\\text{T}^\\text{miss}$ (corr.)}} & {\\rotatebox{90}{b\&c jets}} "
  // 	 <<"& {\\rotatebox{90}{Renormalization}}  & {\\rotatebox{90}{Factorization}} "
  //   	 <<"& {\\rotatebox{90}{$p_{\rm T}$ (top)}}  & {\\rotatebox{90}{top quark mass}}  & {\\rotatebox{90}{$h_{\\text{damp}}$ (shape)}}  "
...
string GetDifferential(string rowtitle, int ifile, bool inc, int year, bool isKFL, bool isInc, char cType, bool isMu)
{
  ...
  //Pileup(0), Lepton(1), bctag1(2), bctag2(3), bctag3(4), Prefire(5), JEC(6), JER(7), facto(8), renorm(9), top-pt-reweight(10), PDF(11), ISR(12), FSR(13), PU JetID(14), CP5(15)/Norm(15), mtop(16)/Stat(16), hdamp(17), Norm(18), Statistical(19)
    ...
    // outFile<<" Category & Process &  {\\rotatebox{90}{Pileup}} "
    // 	 <<"& {\\rotatebox{90}{jet \\& $p_\\text{T}^\\text{miss}$ (corr.)}} & {\\rotatebox{90}{b\&c jets}} "
    // 	 <<"& {\\rotatebox{90}{$p_{\rm T}$ (top)}}  "
    // 	 <<"& {\\rotatebox{90}{Normalization}}  & {\\rotatebox{90}{Statistical}} \\\\ "
    // 	 <<""<<endl;
    
  double jet_met_mu = TMath::Sqrt(muErr[6]*muErr[6] + muErr[7]*muErr[7]);
  double jet_met_ele = TMath::Sqrt(eleErr[6]*eleErr[6] + eleErr[7]*eleErr[7]);
  double bcjets_mu = TMath::Sqrt(muErr[2]*muErr[2] + muErr[3]*muErr[3] + muErr[4]*muErr[4]);
  double bcjets_ele = TMath::Sqrt(eleErr[2]*eleErr[2] + eleErr[3]*eleErr[3] + eleErr[4]*eleErr[4]);

  string combined = "";
  cout<<"Diff sample " << sampleType[ifile-1] <<", RowTitle : " << rowtitle << endl;
  string sampleName = sampleType[ifile-1];
  string nomalization = Form("$%2.1f(%2.1f)$",muErr[15],eleErr[15]);
  if(sampleName.find("TTbar")!=string::npos)
    nomalization = Form("$%2.1f(%2.1f)$",muErr[18],eleErr[18]);
      
  string stat = Form("$%2.1f(%2.1f)$",muErr[15],eleErr[15]);
  if(sampleName.find("TTbar")!=string::npos)
    stat = Form("$%2.1f(%2.1f)$",muErr[19],eleErr[19]);

  if(sampleName.find("QCDdd")==string::npos){
    if(sampleName.find("TTbar")!=string::npos or sampleName.find("Hplmi")!=string::npos){
      combined = string(" & ") + rowtitle + " & " + Form("$%2.1f(%2.1f)$",muErr[0],eleErr[0]) 
	+ " & " + Form("$%2.1f(%2.1f)$",jet_met_mu,jet_met_ele) 
	+ " & " + Form("$%2.1f(%2.1f)$",bcjets_mu,bcjets_ele) 
