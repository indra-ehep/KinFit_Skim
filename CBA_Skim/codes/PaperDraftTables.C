/**********************************************************************
 ...
 Purpose    : Tables for paper draft
...
int PaperDraftTables()
{
  ...
  map<string,pair<double,double>> yield[2][ncT][nM], yieldSum[2][nM];
  int mass[nM] = {40, 50, 60, 70, 80, 90, 100, 110, 120, 130, 140, 150, 155, 160};
  int nSyst;
  double syst[2][61][24];
  map<string,int> systMap[2];
  double JESComb[2][24];
  double bctagComb[2][24];
  for(int ich=0; ich<2 ; ich++)
    for(int in=0;in<61;in++)
      for(int isys=0;isys<24;isys++)
	syst[ich][in][isys] = 0.0;
  
  for(int ich=0; ich<2 ; ich++)
    for(int ism=0;ism<24;ism++){
      JESComb[ich][ism] = 0.0;
      bctagComb[ich][ism] = 0.0;
    }

  int ReadDataCard(string, int, string, int, string, const char* [], map<string,map<int,float>>&, map<string,int>&);
  int WriteSystTable(int year, string ch, string cTag, const char* [], map<string,map<int,float>> [], map<string,int> []);//per channel
  int WriteSystTable(int, int, double [][61][24], map<string,int> [], double [][24], double [][24], map<string,pair<double,double>> [][14], const char* [], map<string,map<int,float>> [], map<string,int> []) ;//for paper draft
  int GetYieldResult(string, int, string, int, string, map<string,pair<double,double>>&, map<string,int>);
  int GetYieldSumResult(string, int, string, int, map<string,pair<double,double>>& , map<string,int> );//for draft
  int WriteYieldTable(int year, map<string,pair<double,double>> [][3][14], map<string,int> [][14]);
  int GroupNuisances(string, int&, double [][61][24], map<string,int>&, double [][24], double [][24], const char* [],
		     map<string,map<int,float>> [], map<string,int> [], map<string,map<int,float>> [], map<string,int> [], map<string,map<int,float>> [], map<string,int> []);
  ...
  for(int year=2016; year<=2018 ; year++){
    for(int im=0;im<nM;im++){
      ...
      GetYieldSumResult(indir, year, "mu", mass[im], yieldSum[0][im], chid[0][im]);
    }
    ...
    GroupNuisances("mu", nSyst, syst, systMap[0], JESComb, bctagComb, nuisances, syst_array[0], chid[0], syst_array[1], chid[1], syst_array[2], chid[2]);
    
    for(int ic=0;ic<ncT;ic++){
      for(int im=0;im<nM;im++){
	syst_array[ic][im].clear();
	chid[ic][im].clear();
	for(int iy=0;iy<2;iy++){
	  yield[iy][ic][im].clear();
	}
      }
    }
    ...
    for(int im=0;im<nM;im++){
      ...
      GetYieldSumResult(indir, year, "ele", mass[im], yieldSum[1][im], chid[0][im]);
    }
    ...
    GroupNuisances("ele", nSyst, syst, systMap[1], JESComb, bctagComb, nuisances, syst_array[0], chid[0], syst_array[1], chid[1], syst_array[2], chid[2]);
    
    WriteYieldTable(year, yield, chid);
    WriteSystTable(year, nSyst, syst, systMap, JESComb, bctagComb, yieldSum, nuisances, syst_array[0], chid[0]); 
...
int WriteYieldTable(int year, map<string,pair<double,double>> yield[][3][14], map<string,int> chid[][14])
{
  ...
  ofstream outFile;
  outFile.open("syst/mjjTable_yield_"+to_string(year)+"_paperdraft.tex");
  ...
  outFile<<"\\hline "<<endl;
  PrintYield("\\ttjets", "ttbar", 0, outFile, yield);
  PrintYield("Single \\PQt quark","stop",0,outFile, yield);
  PrintYield("\\wjets","wjet",0,outFile, yield);
  PrintYield("\\dyjets","zjet",0,outFile, yield);
  PrintYield("$\\mathrm{t\\bar{t}}\\PGg$ + jets","ttg",0,outFile, yield);
  PrintYield("$\\mathrm{t\\bar{t}}\\PH$ + jets","tth",0,outFile, yield);
  PrintYield("$\\mathrm{t\\bar{t}}\\PZ$ + jets","ttz",0,outFile, yield);
  PrintYield("$\\mathrm{t\\bar{t}}\\PW$ + jets","ttw",0,outFile, yield);
  PrintYield("VV","vv",0,outFile, yield);
  PrintYield("QCD multijet","qcd",0,outFile, yield);
  PrintYield("All background","bkg",0,outFile, yield);
  PrintYield("Data","data_obs",0,outFile, yield);
  outFile<<"\\hline "<<endl;
...
int PrintYield(string mcname, string chname, int imass, ofstream& outFile, map<string,pair<double,double>> yield[][3][14])
{
  ...
  outFile << mcname << fixed << setprecision(0) 
	  << "& $" << TMath::Nint(yield[0][0][imass][chname].first) << " \\pm " << TMath::Nint(yield[0][0][imass][chname].second) << "$ "
	  << "& $" << TMath::Nint(yield[1][0][imass][chname].first) << " \\pm " << TMath::Nint(yield[1][0][imass][chname].second) << "$ "
	  << "& $" << TMath::Nint(yield[0][1][imass][chname].first) << " \\pm " << TMath::Nint(yield[0][1][imass][chname].second) << "$ "
	  << "& $" << TMath::Nint(yield[1][1][imass][chname].first) << " \\pm " << TMath::Nint(yield[1][1][imass][chname].second) << "$ "
	  << "& $" << TMath::Nint(yield[0][2][imass][chname].first) << " \\pm " << TMath::Nint(yield[0][2][imass][chname].second) << "$ "
	  << "& $" << TMath::Nint(yield[1][2][imass][chname].first) << " \\pm " << TMath::Nint(yield[1][2][imass][chname].second) << "$ "
	  << "\\\\" << endl;
  
  return true;
}

int GroupNuisances(string channel, int &isyst, double systs[][61][24], map<string,int>& systMap, double maxJES[][24], double maxbctag[][24], const char* nuisances[],
		   map<string,map<int,float>> syst_array1[], map<string,int> chid1[],
		   map<string,map<int,float>> syst_array2[], map<string,int> chid2[],
		   map<string,map<int,float>> syst_array3[], map<string,int> chid3[])
{
  const char* JES_nuisances[] = {"absmpfb", "absscl", "absstat", "flavorqcd",
			         "frag", "timepteta", "pudatamc", "puptbb", "puptec1", "puptec2", "pupthf", "puptref", "relfsr", "relbal",
				 "relsample", "reljerec1", "reljerec2", "relptbb", "relptec1", "relptec2", "relpthf", "relstatfsr", "relstathf", "relstatec",
				 "singpiecal", "singpihcal"}; //26

  const char* bctag_nuisances[] = {"bcstat", "bcxdyb", "bcxdyc", "bcxwjc", "bcintp", "bcextp"};//6

  vector<string> nuinames;
  nuinames.assign(nuisances, nuisances+61);

  vector<string> nuiJES;
  nuiJES.assign(JES_nuisances, JES_nuisances+26);

  vector<string> nuibctag;
  nuibctag.assign(bctag_nuisances, bctag_nuisances+6);
  ...
  const int nM = 14;
  int mass[nM] = {40, 50, 60, 70, 80, 90, 100, 110, 120, 130, 140, 150, 155, 160};
  //float systs[61][24]; //14 charged higgs + 9 MC bkg + 1 QCD
  for(int in=0;in<61;in++)
    for(int isys=0;isys<24;isys++)
      systs[chn][in][isys] = 0.0;
  ...
  for(int ism=0;ism<24;ism++){
    maxJES[chn][ism] = 0.0;
    maxbctag[chn][ism] = 0.0;
  }
  for(const auto systnickname : nuinames){
    if(syst_array1[chn][systnickname].size()>0){
      cout << "Nuiname : " << systnickname << endl;

      for(int im=0;im<nM;im++){
	string mName = "WH" + to_string(mass[im]) ;
	// systs[isyst][im] = TMath::Sqrt(syst_array1[im][systnickname][chid1[im][mName]]*syst_array1[im][systnickname][chid1[im][mName]]
	// 			       + syst_array2[im][systnickname][chid2[im][mName]]*syst_array2[im][systnickname][chid2[im][mName]]
	// 			       + syst_array3[im][systnickname][chid3[im][mName]]*syst_array3[im][systnickname][chid3[im][mName]]);
	systs[chn][isyst][im] = TMath::Max(TMath::Max(syst_array1[im][systnickname][chid1[im][mName]],syst_array2[im][systnickname][chid2[im][mName]]),
				      TMath::Max(syst_array2[im][systnickname][chid2[im][mName]],syst_array3[im][systnickname][chid3[im][mName]]));
	if(systs[chn][isyst][im]>0)
	  cout<<"\t mName : "<<mName<<", syst : " << systs[chn][isyst][im] << endl;
	else
	  cout<<"\t mName : "<<mName<<", syst : -- " << endl;
	for(const auto subjes : nuiJES){
	  if(subjes==systnickname)
	    maxJES[chn][im] = (systs[chn][isyst][im]>maxJES[chn][im]) ? systs[chn][isyst][im] : maxJES[chn][im] ;
	}
	for(const auto subbctag : nuibctag){
	  if(subbctag==systnickname)
	    maxbctag[chn][im] = (systs[chn][isyst][im]>maxbctag[chn][im]) ? systs[chn][isyst][im] : maxbctag[chn][im] ;
	}

      }
      for(int ich=1;ich<int(chid1[0].size());ich++){
	for(const auto ch : chid1[0]){
	  if(ch.second == ich){
	    // systs[isyst][ich-1+nM] = TMath::Sqrt(syst_array1[chn][systnickname][ch.second]*syst_array1[chn][systnickname][ch.second]
	    // 					 + syst_array2[chn][systnickname][ch.second]*syst_array2[chn][systnickname][ch.second]
	    // 					 + syst_array3[chn][systnickname][ch.second]*syst_array3[chn][systnickname][ch.second]);
	    systs[chn][isyst][ich-1+nM] = TMath::Max(TMath::Max(syst_array1[0][systnickname][ch.second],syst_array2[0][systnickname][ch.second]),
						TMath::Max(syst_array2[0][systnickname][ch.second],syst_array3[0][systnickname][ch.second]));
	    if(systs[chn][isyst][ich-1+nM]>0)
	      cout<<"\t bkgsignal : "<<ch.first<<", syst : " << systs[chn][isyst][ich-1+nM] << endl;
	    else
	      cout<<"\t bkgsignal : "<<ch.first<<", syst :  -- "  << endl;

	    for(const auto subjes : nuiJES){
	      if(subjes==systnickname and subjes=="flavorqcd"){
		//maxJES[chn][ich-1+nM] = (systs[chn][isyst][ich-1+nM]>maxJES[chn][ich-1+nM]) ? systs[chn][isyst][ich-1+nM] : maxJES[chn][ich-1+nM] ;
		maxJES[chn][ich-1+nM] = systs[chn][isyst][ich-1+nM] ;
	      }
	    }
	    for(const auto subbctag : nuibctag){
	      if(subbctag==systnickname and subbctag=="bcstat"){
		//maxbctag[chn][ich-1+nM] = (systs[chn][isyst][ich-1+nM]>maxbctag[chn][ich-1+nM]) ? systs[chn][isyst][ich-1+nM] : maxbctag[chn][ich-1+nM] ;
		maxbctag[chn][ich-1+nM] = systs[chn][isyst][ich-1+nM] ;
	      }
	    }
  ...
  for(int ism=0;ism<24;ism++){
    if(ism<nM){
      string mName = "WH" + to_string(mass[ism]) ;
      cout<< "ism : " << ism << ", chname : " << mName << ", JES : " << maxJES[chn][ism] << endl ;
      cout<< "ism : " << ism << ", chname : " << mName << ", bctag : " << maxbctag[chn][ism] << endl ;
    }else{
      for(int ich=1;ich<int(chid1[0].size());ich++){
	for(const auto ch : chid1[0]){
	  if(ch.second == ich and ch.second == ism+1-nM){
	    cout<< "ism : " << ism << ", chname " << ch.first << ", JES : " << maxJES[chn][ism] << endl ;
	    cout<< "ism : " << ism << ", chname " << ch.first << ", bctag : " << maxbctag[chn][ism] << endl ;
	  }
	}
      }
    }
...
int WriteSystTable(int year, int isyst, double systs[][61][24], map<string,int> systMap[], double maxJES[][24], double maxbctag[][24], map<string,pair<double,double>> yieldSum[][14], const char* nuisances[], map<string,map<int,float>> syst_array[], map<string,int> chid[])
{
  vector<string> nuinames;
  nuinames.assign(nuisances, nuisances+61);
  
  const char* draft_nuisances[] = {"pileup", "JES", "bctag", "norm_qcd_", "stat", "topPt"};//6  
  vector<string> nuidraft;
  nuidraft.assign(draft_nuisances, draft_nuisances+6);

  int PrintSyst(int year, string systfullname, string systnickname, ofstream& outFile, int isyst, double systs[][61][24], map<string,int> systMap[], double maxJES[][24], double maxbctag[][24], map<string,pair<double,double>> yieldSum[][14], const char* nuisances[], map<string,map<int,float>> syst_array[], map<string,int> chid[]);
    
  ofstream outFile;
  outFile.open("syst/mjjSystTable_"+to_string(year)+"_paperdraft.tex");
  outFile<<"\\documentclass[]{article}"<<endl;
  outFile<<"\\usepackage{amsmath}"<<endl;
  outFile<<"\\usepackage{array}"<<endl;
  outFile<<"\\usepackage{multirow}"<<endl;
  ...
  outFile<<""<<endl;
  outFile<<""<<endl;
  string tab2_caption = "Systematic and statistical uncertainties in \\% for exclusive charm tagging in " + to_string(year) + ".";;
  
  outFile<<"\\begin{table}"<<endl;
  outFile<<"\\centering\\caption{"+tab2_caption+"}"<<endl;
  outFile<<"\\label{tab:sec07_syst_"<<year<<"}"<<endl;
  outFile<<"\\begin{adjustbox}{width=\\textwidth}"<<endl;
  ///outFile<<"\\footnotesize\\setlength{\\tabcolsep}{0.3pt}"<<endl;
  outFile<<"\\begin{tabular}{ ll  ccc cc cccc cccc}"<<endl;
  outFile<<"\\hline "<<endl;  

  outFile<<" Year & Nuisances "
  	 <<"& {\\rotatebox{90}{$\\text{H}^{+} + \\text{H}^{-} (\\text{m}=50$ GeV)}} "
  	 <<"& {\\rotatebox{90}{$\\text{H}^{+} + \\text{H}^{-} (\\text{m}=100$ GeV)}} "
  	 <<"& {\\rotatebox{90}{$\\text{H}^{+} + \\text{H}^{-} (\\text{m}=150$ GeV)}} "
  	 <<"& {\\rotatebox{90}{\\ttjets}} "
  	 <<"& {\\rotatebox{90}{$\\mathrm{t\\bar{t}}\\PGg$ + jets}} "
    	 <<"& {\\rotatebox{90}{$\\mathrm{t\\bar{t}}\\PH$ + jets}} "
  	 <<"& {\\rotatebox{90}{$\\mathrm{t\\bar{t}}\\PZ$ + jets}} "
    	 <<"& {\\rotatebox{90}{$\\mathrm{t\\bar{t}}\\PW$ + jets}} "
  	 <<"& {\\rotatebox{90}{\\wjets}} "
    	 <<"& {\\rotatebox{90}{\\dyjets}} "
  	 <<"& {\\rotatebox{90}{Single \\PQt quark}} "
    	 <<"& {\\rotatebox{90}{VV}} "
  ...
  for(const auto systdraft : nuidraft){
    //cout << "Nuiname : " << systdraft << endl;
    int isys = 0;
    for(const auto systnickname : nuinames){
      if(systnickname==systdraft){
	if(systdraft=="pileup") PrintSyst(year,"Pileup", "pileup", outFile, isys, systs, systMap, maxJES, maxbctag, yieldSum, nuisances, syst_array, chid);
	if(systdraft.find("norm")!=string::npos) PrintSyst(year,"Normalization", "norm_qcd_", outFile, isys, systs, systMap, maxJES, maxbctag, yieldSum, nuisances, syst_array, chid);
	if(systdraft=="topPt") PrintSyst(year,"$p_{\\rm T} (\\text{t})$", "topPt", outFile, isys, systs, systMap, maxJES, maxbctag, yieldSum, nuisances, syst_array, chid);
      }
      isys++;
    }
    if(systdraft=="JES") PrintSyst(year,"jet \\& $p_{\\rm T}^{\\rm miss}$", "JES", outFile, isys, systs, systMap, maxJES, maxbctag, yieldSum, nuisances, syst_array, chid);
    if(systdraft=="bctag") PrintSyst(year,"$b$ \\& $c$ jets", "bctag", outFile, isys, systs, systMap, maxJES, maxbctag, yieldSum, nuisances, syst_array, chid);
    if(systdraft=="stat") PrintSyst(year,"Statistical", "stat", outFile, isys, systs, systMap, maxJES, maxbctag, yieldSum, nuisances, syst_array, chid);
  }
...
int PrintSyst(int year, string systfullname, string systnickname, ofstream& outFile, int isys, double systs[][61][24], map<string,int> systMap[], double maxJES[][24], double maxbctag[][24], map<string,pair<double,double>> yieldSum[][14], const char* nuisances[], map<string,map<int,float>> syst_array[], map<string,int> chid[])
{
  cout << "hello there : " << systnickname << endl;
  isys = systMap[0][systnickname];
  if(systnickname.find("norm_qcd_")!=string::npos){
    isys = systMap[0]["norm_qcd_"];
    float qcd_syst_mu = 0;
    cout << "Before qcd_syst : " << qcd_syst_mu << endl;
    qcd_syst_mu = systs[0][isys][23] ;
    cout << "After qcd_syst : " << qcd_syst_mu << endl;
    float qcd_syst_ele = 0;
    cout << "Before qcd_syst : " << qcd_syst_ele << endl;
    qcd_syst_ele = systs[1][isys][23] ;
    cout << "After qcd_syst : " << qcd_syst_ele << endl;
    outFile << "& " << systfullname
	    << "& 6.1 (6.1) " << "& 6.1 (6.1)" << "& 6.1 (6.1)" << "& 6.1 (6.1)" << "& 6.1 (6.1)" << "& 6.1 (6.1)" << "& 6.1 (6.1)" << "& 6.1 (6.1)"
	    << "& 4.5 (4.5)"<< "& 5.0 (5.0)"<< "& 7.0 (7.0)"<< "& 4.0 (4.0)"<<"& "<<qcd_syst_mu << " ( " << qcd_syst_ele <<") \\\\"<<endl;
  }else if(systnickname.find("JES")!=string::npos){
    outFile << "& " << systfullname << fixed << setprecision(1) ;
    outFile << "&" << maxJES[0][1] << " (" << maxJES[1][1] << ")";
    outFile << "&" << maxJES[0][6] << " (" << maxJES[1][6] << ")";
    outFile << "&" << maxJES[0][11] << " (" << maxJES[1][11] << ")";
    for(int is=14;is<24;is++){
      if(maxJES[0][is]>0.0)
    	outFile << "& " << maxJES[0][is] << " (" << maxJES[1][is] << ")";
      else
    	outFile << "& - ";
    }
   outFile<<" \\\\"<<endl;
    
  }else if(systnickname.find("bctag")!=string::npos){
    outFile << "& " << systfullname << fixed << setprecision(1) ;
    outFile << "&" << maxbctag[0][1] << " (" << maxbctag[1][1] << ")";
    outFile << "&" << maxbctag[0][6] << " (" << maxbctag[1][6] << ")";
    outFile << "&" << maxbctag[0][11] << " (" << maxbctag[1][11] << ")";
    for(int is=14;is<24;is++){
      if(maxbctag[0][is]>0.0)
    	outFile << "& " << maxbctag[0][is] << " (" << maxbctag[1][is] << ")";
      else
    	outFile << "& - ";
    }
   outFile<<" \\\\"<<endl;
    
  }else if(systnickname.find("stat")!=string::npos){
    outFile << "& " << systfullname << fixed << setprecision(1) ;
    outFile << "&" << yieldSum[0][1]["WH50"].second << " (" << yieldSum[1][1]["WH50"].second << ")";
    outFile << "&" << yieldSum[0][6]["WH100"].second << " (" << yieldSum[1][6]["WH100"].second << ")";
    outFile << "&" << yieldSum[0][11]["WH150"].second << " (" << yieldSum[1][11]["WH150"].second << ")";
    outFile << "&" << yieldSum[0][0]["ttbar"].second << " (" << yieldSum[1][0]["ttbar"].second << ")";
    outFile << "&" << yieldSum[0][0]["ttg"].second << " (" << yieldSum[1][0]["ttg"].second << ")";
    outFile << "&" << yieldSum[0][0]["tth"].second << " (" << yieldSum[1][0]["tth"].second << ")";
    outFile << "&" << yieldSum[0][0]["ttz"].second << " (" << yieldSum[1][0]["ttz"].second << ")";
    outFile << "&" << yieldSum[0][0]["ttw"].second << " (" << yieldSum[1][0]["ttw"].second << ")";
    outFile << "&" << yieldSum[0][0]["wjet"].second << " (" << yieldSum[1][0]["wjet"].second << ")";
    outFile << "&" << yieldSum[0][0]["zjet"].second << " (" << yieldSum[1][0]["zjet"].second << ")";
    outFile << "&" << yieldSum[0][0]["stop"].second << " (" << yieldSum[1][0]["stop"].second << ")";
    outFile << "&" << yieldSum[0][0]["vv"].second << " (" << yieldSum[1][0]["vv"].second << ")";
    outFile << "& - ";
    outFile<<" \\\\"<<endl;
    
  }else{
    if(systnickname.find("pileup")!=string::npos)
     outFile << year << "& " << systfullname << fixed << setprecision(1) ;
    else
     outFile << "& " << systfullname << fixed << setprecision(1) ;
    outFile << "&" << systs[0][isys][1] << " (" << systs[1][isys][1] << ")";
    outFile << "&" << systs[0][isys][6] << " (" << systs[1][isys][6] << ")";
    outFile << "&" << systs[0][isys][11] << " (" << systs[1][isys][11] << ")";
    for(int is=14;is<24;is++){
      if(systs[0][isys][is]>0.0)
    	outFile << "& " << systs[0][isys][is] << " (" << systs[1][isys][is] << ")";
      else
    	outFile << "& - ";
    }
   outFile<<" \\\\"<<endl;
...
int WriteSystTable(int year, string ch, string cTag, const char* nuisances[], map<string,map<int,float>> syst_array[], map<string,int> chid[])
{
  ...
  ofstream outFile;
  outFile.open("syst/mjjTable_"+ch+"_"+cTag+"_"+to_string(year)+"_paperdraft.tex");
  outFile<<"\\documentclass[]{article}"<<endl;
  outFile<<"\\usepackage{amsmath}"<<endl;
  outFile<<"\\usepackage{array}"<<endl;
  outFile<<"\\usepackage{multirow}"<<endl;
...
int GetYieldResult(string indir, int year, string ch, int mass, string ctag, map<string,pair<double,double>>& yield, map<string,int> chid)
{
  ...
  double bkg_integral = 0.0, bkg_error = 0.0;
  for(const auto ch : chid){
    ...
    if(ch.first.find("WH")==string::npos){
      bkg_integral += double(TMath::Nint(integral));
      bkg_error += error*error;
    }
    //cout << "Name : " << h1->GetName() << ", Integral : " << integral << " +/- " << error << endl;
  }
  string ch_data = "data_obs";
  TH1F *h1 = (TH1F *)fin->Get(ch_data.c_str());
  double error;
  double integral = h1->IntegralAndError(1, h1->GetNbinsX(), error);
  yield[ch_data] = make_pair(integral,error);

  string ch_bkg = "bkg";
  bkg_error = TMath::Sqrt(bkg_error);
  yield[ch_bkg] = make_pair(bkg_integral,bkg_error);
...
int GetYieldSumResult(string indir, int year, string ch, int mass, map<string,pair<double,double>>& yield, map<string,int> chid)
{

  string infile1="limit/TIFRAPAR-2023-01-15/"+indir+"/"+to_string(year)+"/Comb/"+ch+"/Cat1_Inc/Mass"+to_string(mass)+"/Shapes_hcs_13TeV_"+ch+"___ct_ExcL_mjj_"+ch+"_WH"+to_string(mass)+"_"+to_string(year)+".root";
  string infile2="limit/TIFRAPAR-2023-01-15/"+indir+"/"+to_string(year)+"/Comb/"+ch+"/Cat1_Inc/Mass"+to_string(mass)+"/Shapes_hcs_13TeV_"+ch+"___ct_ExcM_mjj_"+ch+"_WH"+to_string(mass)+"_"+to_string(year)+".root";
  string infile3="limit/TIFRAPAR-2023-01-15/"+indir+"/"+to_string(year)+"/Comb/"+ch+"/Cat1_Inc/Mass"+to_string(mass)+"/Shapes_hcs_13TeV_"+ch+"___ct_ExcT_mjj_"+ch+"_WH"+to_string(mass)+"_"+to_string(year)+".root";
  
  cout << "Infile1 : " << infile1 << endl;
  cout << "Infile2 : " << infile2 << endl;
  cout << "Infile3 : " << infile3 << endl;
  ...
  double bkg_integral = 0.0, bkg_error = 0.0;
  for(const auto ch : chid){
    TH1F *h1 = (TH1F *)fin1->Get(ch.first.c_str());
    TH1F *h2 = (TH1F *)fin2->Get(ch.first.c_str());
    TH1F *h3 = (TH1F *)fin3->Get(ch.first.c_str());
    h1->Add(h2);
    h1->Add(h3);
    double error;
    double integral = h1->IntegralAndError(1, h1->GetNbinsX(), error);
    error = 100.*error/integral;
    yield[ch.first] = make_pair(integral,error);
    if(ch.first.find("WH")==string::npos){
      bkg_integral += double(TMath::Nint(integral));
      bkg_error += error*error;
  ...
  string ch_data = "data_obs";
  TH1F *h1 = (TH1F *)fin1->Get(ch_data.c_str());
  TH1F *h2 = (TH1F *)fin2->Get(ch_data.c_str());
  TH1F *h3 = (TH1F *)fin3->Get(ch_data.c_str());
  h1->Add(h2);
  h1->Add(h3);
  double error;
  double integral = h1->IntegralAndError(1, h1->GetNbinsX(), error);
