...
grep -i -E 'break|crash|segmentation|error' *.stderr | grep -v -E "TDecompLU|unknown\ branch" > $errorfile
grep -i -E 'break|crash|segmentation|error' *.stderr | grep -v -E "TDecompLU|unknown\ branch" | cut -f 1 -d '.' > $outputfile
echo "See the errors in  $errorfile"
echo "The list of files with error reports are saved in $outputfile"
