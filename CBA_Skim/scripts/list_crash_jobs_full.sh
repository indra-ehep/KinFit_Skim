...
grep -i -E 'break|crash|segmentation|error|killed' *.stderr | grep -v -E "TDecompLU|unknown\ branch|GenJet\_hadronFlavour|A\ list\ is\ accessing\ an\ object|Local\ error:\ file\ exists" > $errorfile
grep -i -E 'break|crash|segmentation|error|killed' *.stderr | grep -v -E "TDecompLU|unknown\ branch|GenJet\_hadronFlavour|A\ list\ is\ accessing\ an\ object|Local\ error:\ file\ exists" | cut -f 1 -d '.' > $outputfile
...
if [ $noflines -gt 0 ]; then 
    ...
    submitjdl=`ls ../submitJobs*.jdl | head -n 1`
    head -n 18 $submitjdl > $resubmitjdl  
