...
grep -i -E 'break|crash|segmentation|error|killed' *.stderr | grep -v -E "TDecompLU|unknown\ branch|GenJet\_hadronFlavour|A\ list\ is\ accessing\ an\ object" > $errorfile
grep -i -E 'break|crash|segmentation|error|killed' *.stderr | grep -v -E "TDecompLU|unknown\ branch|GenJet\_hadronFlavour|A\ list\ is\ accessing\ an\ object" | cut -f 1 -d '.' > $outputfile
