...
Bool_t SkimAna::Process(Long64_t entry)
{
  ...
  if(!isData){ //To process for LHE, PYTHIA and GenJets
    TheoWeights();
    if(fSampleType.Contains("TTbar")){
      if (overlapRemoval(event, 10., 5., 0.1, false))
	return true;
    }
    if(fSampleType.Contains("TTGTo")){
      if (!overlapRemoval(event, 10., 5., 0.1, false))
	return true;
    }
    //   // if(!SelectTTbarChannel()) return true;
...
//_____________________________________________________________________________
std::vector<double> SkimAna::minGenDr(int myInd, const EventTree* tree, std::vector<int> ignorePID){
...
//_____________________________________________________________________________
bool SkimAna::overlapRemoval(EventTree* tree, double Et_cut, double Eta_cut, double dR_cut, bool verbose){
