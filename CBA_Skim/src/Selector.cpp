...
void Selector::filter_muons(){
  ...
  for(int muInd = 0; muInd < int(tree->nMuon_); ++muInd){
    ...
    if(TMath::Abs(charge)!=1 or nTrackerLayers>=20 or nTrackerLayers<0 or !(TMath::Finite(eta)) or !(TMath::Finite(pt)) or !(TMath::Finite(phi)) 
...
void Selector::filter_jets(){
  ...
  for(int jetInd = 0; jetInd < int(tree->nJet_) and int(tree->nJet_) < 10000000 ; ++jetInd){ //less than 10 million jets since some MCQCD shows unexceptionally number of higher jets
    ...
    //bool jetID_pass = (tree->jetID_[jetInd]>=2 and (tree->jetPUID_[jetInd]>=2 or pt>=50.0)) ; //(tree->jetID_[jetInd]>=2) ;
    //Suggested by RBI
