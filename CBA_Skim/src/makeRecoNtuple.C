#include "../interface/makeRecoNtuple.h"
#define makeRecoNtuple_cxx
...
#ifdef makeRecoNtuple_cxx
makeRecoNtuple::makeRecoNtuple(int ac, char** av)
{
    startClock = std::chrono::high_resolution_clock::now();
    std::string eventStr = "-1";
    if(ac < 5){
	std::cout << "usage: ./makeRecoNtuple year sampleName outputFileDir inputFile[s]" << std::endl;
	return;
    }
...
//void makeRecoNtuple::FillEvent(std::string year)
void makeRecoNtuple::FillEvent(std::string year, bool isHemVetoObj) //HEM test
{
    _run             = tree->run_;
    _event           = tree->event_;
    _lumis           = tree->lumis_;
