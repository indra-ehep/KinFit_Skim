...
samples_2016 = ["TTbar"]
# samples_2017 = ["TTbar", "HplusM080", "HplusM100", "HplusM120", "HplusM140", "HplusM160"]
# samples_2018 = ["TTbar"]
...
if not os.path.exists("tmplog_deepjet_binchange_post_it1/log"):
    os.makedirs("tmplog_deepjet_binchange_post_it1/log")
condorLogDir = "log"
tarFile = "tmplog_deepjet_binchange_post_it1/CBA_Skim.tar.gz"
if os.path.exists(tarFile):
	os.system("rm %s"%tarFile)
os.system("tar -zcvf %s ../../CBA_Skim --exclude condor"%tarFile)
os.system("cp runCBASkim.sh tmplog_deepjet_binchange_post_it1/")
common_command = \
'Universe   = vanilla\n\
...
#Create jdl files
#----------------------------------------
subFile = open('tmplog_deepjet_binchange_post_it1/condorSubmit.sh','w')
#for year in [2017,2018]:
for year in [2016]:
#for year in [2017]:
    sampleList = eval("samples_%i"%year)
    jdlName = 'submitJobs_%s.jdl'%(year)
    jdlFile = open('tmplog_deepjet_binchange_post_it1/%s'%jdlName,'w')
    jdlFile.write('Executable =  runCBASkim.sh \n')
    jdlFile.write(common_command)
    condorOutDir="/eos/user/s/savarghe/Indra_Da/Output/cms-hcs-run2/CBA_Deepjet_NewBin/post"
    os.system("eos root://eosuser.cern.ch mkdir -p %s/%s"%(condorOutDir, year))
    condorOutDir1="/eos/user/i/idas/Output/cms-hcs-run2/CBA_Deepjet_NewBin/post"
