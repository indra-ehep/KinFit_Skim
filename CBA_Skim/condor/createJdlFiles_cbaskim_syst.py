...
if not os.path.exists("tmplog_MiniAODEqV_post_it3/log"):
    os.makedirs("tmplog_MiniAODEqV_post_it3/log")
condorLogDir = "log"
tarFile = "tmplog_MiniAODEqV_post_it3/CBA_Skim.tar.gz"
if os.path.exists(tarFile):
	os.system("rm %s"%tarFile)
os.system("tar -zcvf %s ../../CBA_Skim --exclude condor"%tarFile)
os.system("cp runCBASkim.sh tmplog_MiniAODEqV_post_it3/")
common_command = \
'Universe   = vanilla\n\
...
#Create jdl files
#----------------------------------------
subFile = open('tmplog_MiniAODEqV_post_it3/condorSubmit.sh','w')
#for year in [2017,2018]:
for year in [2016]:
#for year in [2017]:
    sampleList = eval("samples_%i"%year)
    jdlName = 'submitJobs_%s.jdl'%(year)
    jdlFile = open('tmplog_MiniAODEqV_post_it3/%s'%jdlName,'w')
