...
if not os.path.exists("tmplog_iter2/log"):
    os.makedirs("tmplog_iter2/log")
condorLogDir = "log"
tarFile = "tmplog_iter2/CBA_Skim.tar.gz"
if os.path.exists(tarFile):
	os.system("rm %s"%tarFile)
os.system("tar -zcvf %s ../../CBA_Skim --exclude condor"%tarFile)
os.system("cp runCBASkim.sh tmplog_iter2/")
common_command = \
'Universe   = vanilla\n\
...
#Create jdl files
#----------------------------------------
subFile = open('tmplog_iter2/condorSubmit.sh','w')
for year in [2016]:
    sampleList = eval("samples_%i"%year)
    jdlName = 'submitJobs_%s.jdl'%(year)
    jdlFile = open('tmplog_iter2/%s'%jdlName,'w')
    ...
    condorOutDir1="/eos/user/i/idas/Output/cms-hcs-run2/CBA_Skim_Syst_ULv9_jetpuid_lwp"
