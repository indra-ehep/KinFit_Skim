...
if not os.path.exists("tmplog_tune_post_t2/log"):
    os.makedirs("tmplog_tune_post_t2/log")
condorLogDir = "log"
tarFile = "tmplog_tune_post_t2/CBA_Skim.tar.gz"
if os.path.exists(tarFile):
	os.system("rm %s"%tarFile)
os.system("tar -zcvf %s ../../CBA_Skim --exclude condor"%tarFile)
os.system("cp runCBASkim.sh tmplog_tune_post_t2/")
common_command = \
'Universe   = vanilla\n\
...
#Create jdl files
#----------------------------------------
subFile = open('tmplog_tune_post_t2/condorSubmit.sh','w')
#for year in [2017,2018]:
for year in [2016]:
    sampleList = eval("samples_%i"%year)
    jdlName = 'submitJobs_%s.jdl'%(year)
    jdlFile = open('tmplog_tune_post_t2/%s'%jdlName,'w')
    jdlFile.write('Executable =  runCBASkim.sh \n')
    jdlFile.write(common_command)
    condorOutDir="/eos/user/s/savarghe/Indra_Da/Output/cms-hcs-run2/CBA_KFNewReso/post"
    os.system("eos root://eosuser.cern.ch mkdir -p %s/%s"%(condorOutDir, year))
    condorOutDir1="/eos/user/i/idas/Output/cms-hcs-run2/CBA_CTagM/post"
        ...
        for syst in systList:
            if (syst.find('cp5')>=0 or syst.find('hdamp')>=0 or syst.find('mtop')>=0):
                if sample.find('TTbar') >=0:
                    noflines = subprocess.Popen('wc -l ../input/eos/%i/post/%s_%i.txt | awk \'{print $1}\''%(year,tunedict[syst],year),shell=True,stdout=subprocess.PIPE).communicate()[0].split('\n')[0]
                    nJob = int(noflines)
                ...
                noflines = subprocess.Popen('wc -l ../input/eos/%i/post/%s_%i.txt | awk \'{print $1}\''%(year,sample,year),shell=True,stdout=subprocess.PIPE).communicate()[0].split('\n')[0]
                nJob = int(noflines)
                print "%s %s %s"%(sample,nJob,syst)
                if nJob==1:
