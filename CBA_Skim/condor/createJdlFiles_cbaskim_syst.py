...
if not os.path.exists("tmplog_CFGenWt_post_it2/log"):
    os.makedirs("tmplog_CFGenWt_post_it2/log")
condorLogDir = "log"
tarFile = "tmplog_CFGenWt_post_it2/CBA_Skim.tar.gz"
if os.path.exists(tarFile):
	os.system("rm %s"%tarFile)
os.system("tar -zcvf %s ../../CBA_Skim --exclude condor"%tarFile)
os.system("cp runCBASkim.sh tmplog_CFGenWt_post_it2/")
common_command = \
'Universe   = vanilla\n\
...
#Create jdl files
#----------------------------------------
subFile = open('tmplog_CFGenWt_post_it2/condorSubmit.sh','w')
#for year in [2017,2018]:
for year in [2016]:
#for year in [2017]:
    sampleList = eval("samples_%i"%year)
    jdlName = 'submitJobs_%s.jdl'%(year)
    jdlFile = open('tmplog_CFGenWt_post_it2/%s'%jdlName,'w')
