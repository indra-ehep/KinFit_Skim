...
if not os.path.exists("tmpSubSystSlStudy6/log"):
    os.makedirs("tmpSubSystSlStudy6/log")
condorLogDir = "log"
tarFile = "tmpSubSystSlStudy6/CBA_Skim.tar.gz"
if os.path.exists(tarFile):
	os.system("rm %s"%tarFile)
os.system("tar -zcvf %s ../../CBA_Skim --exclude condor"%tarFile)
os.system("cp runCBASkim.sh tmpSubSystSlStudy6/")
common_command = \
'Universe   = vanilla\n\
...
#Create jdl files
#----------------------------------------
subFile = open('tmpSubSystSlStudy6/condorSubmit.sh','w')
for year in [2016,2017,2018]:
    sampleList = eval("samples_%i"%year)
    jdlName = 'submitJobs_%s.jdl'%(year)
    jdlFile = open('tmpSubSystSlStudy6/%s'%jdlName,'w')
    jdlFile.write('Executable =  runCBASkim.sh \n')
    jdlFile.write(common_command)
    condorOutDir1="/eos/user/i/imirza/idas/Output/cms-hcs-run2/CBA_Skim_Syst_SlStudy6"
