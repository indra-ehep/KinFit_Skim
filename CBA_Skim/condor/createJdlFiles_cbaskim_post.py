...
os.system("cp runCBASkim_post.sh %s/"%jdlDir)
common_command = \
'Universe   = vanilla\n\
should_transfer_files = YES\n\
when_to_transfer_output = ON_EXIT\n\
Transfer_Input_Files = CBA_Skim.tar.gz, runCBASkim_post.sh\n\
...
for year in [2016]:
    ...
    jdlFile.write('Executable =  runCBASkim_post.sh \n')
    jdlFile.write(common_command)
    # condorOutDir="/eos/user/s/savarghe/Indra_Da/Output/cms-hcs-run2/CBA_lowjetpt-highmet/post"
    # condorOutDir2="/cms/store/user/idas/Output/cms-hcs-run2/KinTreeUL/CBA_lowjetpt-highmet/post"
    ...
    condorOutDir1="/eos/user/i/idas/Output/cms-hcs-run2/CBA_lowjetpt-highmet/post"
    os.system("eos root://eosuser.cern.ch mkdir -p %s/%s"%(condorOutDir1, year))
    condorOutDir3="/eos/user/d/dugad/idas/Output/cms-hcs-run2/CBA_lowjetpt-highmet/post"
    os.system("eos root://eosuser.cern.ch mkdir -p %s/%s"%(condorOutDir3, year))
    condorOutDir4="/eos/cms/store/group/phys_b2g/idas/Output/cms-hcs-run2/Result/CBA_lowjetpt-highmet/post"
