...
outdir=signif_qcdrange0to50_qcddd6_bc3_uncorr_br0p0010to0p0110_sym_nzlbin_10GeV_grid100_iter$iloop
...
cd $testdir
rsync -avP  /home/hep/idas/temp/condor_test/test_globalsig_condor/tmpLog_globalsig_iter$iloop/local.tar.gz .
tar -zxf local.tar.gz
cd local/$year/$cattype/$channel/Cat1_Inc/Mass$mass
source condor_run.sh $itoy > out_${itoy}.log 2>&1
rsync -avP out_${itoy}.log $condorOutDir
rsync -avP higgsCombine_bfit_toy${itoy}.MultiDimFit.mH60.root $condorOutDir
rsync -avP higgsCombine_data_toy${itoy}.MultiDimFit.mH60.root $condorOutDir
cd $testdir
