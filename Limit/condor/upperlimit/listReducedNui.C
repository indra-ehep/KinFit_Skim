...
int main(int argc, char **argv)
{
  ...
  for (YAML::const_iterator it = node.begin(); it != node.end(); ++it) {
	...
	double postfitbyprefit_sigma =  fabs(postfitval - prefitval)/fabs(prefitmax - prefitval) ;
	  
	nuipobypre[value1[i]["name"].as<std::string>()] = postfitbyprefit_sigma;
	reducedlist[postfitbyprefit_sigma] = value1[i]["name"].as<std::string>() ;
	...
	// }
      ...
      //std::map<double,std::string>::iterator itim;
      int iloop = 0;
      //for(itim=impactlist.rbegin();itim!=impactlist.rend() and iloop<firstNNui ;itim++){
      for(itim=reducedlist.rbegin();itim!=reducedlist.rend() and iloop<firstNNui ;itim++){
	...
	if(nuiname.find("prop_binch")!=std::string::npos) continue;
	//if(nuipobypre[nuiname]<2.0 or iloop>=firstNNui) continue;
