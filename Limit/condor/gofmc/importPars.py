...
def importPars(incard,fitresults,toDrop):
    ...
    subprocess.call(['text2workspace.py '+incard+' -P HiggsAnalysis.CombinedLimit.ChargedHiggs:brChargedHiggs -o morphedWorkspace.root'],shell=True)
    ...
    for fr_name in fitresults:
        ...
        print ('FileName : %s'%(fr_f.GetName()))
        fr = fr_f.Get('fit_b') # b-only fit result (fit_s for s+b)
        print ('frName : %s'%(fr.GetName()))
