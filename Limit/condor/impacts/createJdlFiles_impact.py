...
jdlDir = 'tmpLog_impact_iter%s'%iloop
if not os.path.exists("%s/log"%jdlDir):
    os.makedirs("%s/log"%jdlDir)
condorLogDir = "log"
os.system("cp runImpact.sh %s/"%jdlDir)
os.system("cp listReducedNui.exe %s/"%jdlDir)
os.system("cp genImpactCondor.sh %s/"%jdlDir)
common_command = \
'executable = runImpact.sh\n\
Transfer_Input_Files = local.tar.gz\n\
+MaxRuntime = 10799\n\
request_memory = 8000\n\
...
for year in yearlist:
    for channel in channellist:
        for mass in masslist:
            os.system("sh genImpactCondor.sh %s %s %s" %(mass, year, channel))
...
for year in yearlist:
    for mass in masslist:
        jdlName = 'submitJobs_%s_%s.jdl'%(year,mass)
        ...
        for channel in channellist:
            ...
            run_command =  'Arguments  = %s %s %s $(process) %s\nQueue %s\n\n' %(mass,year,channel,iloop,nqueue)
