...
void LimitPlotter(TString CHANNEL="mu", TString CAT= "Cat1_Inc",
  ...
  //TString year_dir = "MiniAODTemplate/MiniAOD_Default/";
  //TString year_dir = "MiniAODTemplate/MiniAOD_NoTopPtReweight/";
  TString year_dir = "";
  for(unsigned int i = 0 ; i < UInt_t(nMassPoints); i++){
    ///cout<<"limit/"+CHANNEL+"/"+CAT+"/"<<massFiles[i]<<endl;
    //TFile f("local/"+CHANNEL+"/"+CAT+"/"+massFiles[i],"READ"); // higgsCombine_hcs_13TeV_mu.AsymptoticLimits.mH140.root
    TFile f("local/"+year_dir+CHANNEL+"/"+CAT+"/"+massFiles[i],"READ"); // higgsCombine_hcs_13TeV_mu.AsymptoticLimits.mH140.root
    if(f.IsZombie()){
  ...
  for(int i1 = 0 ; i1 < nMassPoints ; i1++){
    ...
    //cout<<"$"<<std::setprecision(3)<<expY[i1]<<"^{+"<<expY1sH_[i1]<<"}"<<"_"<<"{-"<< expY1sL_[i1]<<"}"<<"$"<<endl;
