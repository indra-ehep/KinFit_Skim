...
void MyHPlusDataCardMakerNano(TString inFileDir="stack_20180418_Mu_Sys_PreAppComent", 
  ...
  */
  ...
  TH1F* wh_bin_stat_Up = 0x0;
  TH1F* wh_bin_stat_Down = 0x0;
  for(int ibin = 1; ibin <=50 ; ibin++){
    //if(ibin == 9 and ibin <=14) continue;
    //if(ibin >= 27 and ibin <=34) continue;
    wh_bin_stat_Up = DC.readWriteHisto(fWH, baseDir+"/base"+histSubDir, histName, sf_wh, fout, fTT, label+Form("_CMS_stat_cat1_WH_bin_%dUp",ibin), true);
    wh_bin_stat_Down = DC.readWriteHisto(fWH, baseDir+"/base"+histSubDir, histName, sf_wh, fout, fTT, label+Form("_CMS_stat_cat1_WH_bin_%dDown",ibin), true);
  }
