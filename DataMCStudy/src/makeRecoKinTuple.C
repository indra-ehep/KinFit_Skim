#include "../interface/makeRecoKinTuple.h"
#define makeRecoKinTuple_cxx
...
#ifdef makeRecoKinTuple_cxx
makeRecoKinTuple::makeRecoKinTuple(int ac, char** av)
{
  startClock = std::chrono::high_resolution_clock::now();
  std::string eventStr = "-1";
  if(ac < 5){
    std::cout << "usage: ./makeRecoKinTuple year sampleName outputFileDir inputFile[s]" << std::endl;
    return;
  }
  ...
  cout << "sampleType : " << sampleType << ",  systematicType : " << systematicType << endl;
  ...
  if (year=="2016"){
    ...
    // PUfilename      = "weight/PileupSF/New/PU_2016_35920_XSecCentral.root";
    // PUfilename_up   = "weight/PileupSF/New/PU_2016_35920_XSecUp.root";
    // PUfilename_down = "weight/PileupSF/New/PU_2016_35920_XSecDown.root";
  }
  if (year=="2017"){
    ...
    // PUfilename      = "weight/PileupSF/New/PU_2017_41530_XSecCentral.root";
    // PUfilename_up   = "weight/PileupSF/New/PU_2017_41530_XSecUp.root";
    // PUfilename_down = "weight/PileupSF/New/PU_2017_41530_XSecDown.root";
  }
  if (year=="2018"){
    ...
    // PUfilename      = "weight/PileupSF/New/PU_2018_59740_XSecCentral.root";
    // PUfilename_up   = "weight/PileupSF/New/PU_2018_59740_XSecUp.root";
    // PUfilename_down = "weight/PileupSF/New/PU_2018_59740_XSecDown.root";
  }
  if (eventNum > -1) {
  ...
  if (isMC){
    ...
    //if (year=="2016") selector->init_JER("weight/JetSF/JER/Summer20UL16_JRV3"); // For MiniAOD comparison
...
//void makeRecoKinTuple::FillEvent(std::string year)
void makeRecoKinTuple::FillEvent(std::string year, bool isHemVetoObj) //HEM test
{
  _run             = tree->run_;
  _event           = tree->event_;
  _lumis           = tree->lumis_;
  ...
  for (int i_genJet = 0; i_genJet < _nGenJet; i_genJet++){
    ...
    _genJetPartonFlavour.push_back(tree->GenJet_partonFlavour_[i_genJet]);
    _genJetHadronFlavour.push_back(tree->GenJet_hadronFlavour_[i_genJet]);
  }
  
}

// https://twiki.cern.ch/twiki/bin/viewauth/CMS/TopPtReweighting
double makeRecoKinTuple::SFtop(double pt){
    return exp(0.0615 - 0.0005*pt);
}

double makeRecoKinTuple::topPtWeight(){
    double toppt=0.0;
...
void makeRecoKinTuple::loadBtagEff(string sampleName, string year){
